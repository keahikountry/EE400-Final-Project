...
/* Includes ---------------------------------------------------------------- */
#include <FinalProject3_inferencing.h>
...
void loop()
{
    ...
    // Print the prediction results (classification)
        ...
        // If your "human" class is index 0 (as your code implies), keep this:
        if (result.classification[0].value > 0.5f) {
            ...
            const uint32_t nbytes = w * h * 2;

            // Print a header with width, height, and format
            Serial.print("BEGIN_FRAME_HEX ");
            Serial.print(w);
            Serial.print(" ");
            Serial.print(h);
            Serial.print(" RGB565 ");
            ...
            for (uint32_t i = 0; i < nbytes; i += 2) {
                Serial.print("0x");
                if (snapshot_buf[i] < 16) Serial.print("0"); // zero pad
                Serial.print(snapshot_buf[i], HEX);
                if (snapshot_buf[i+1] < 16) Serial.print("0");
                Serial.print(snapshot_buf[i+1], HEX);

                if (i < nbytes - 2) Serial.print(", ");
            }
            Serial.println();
            Serial.println("END_FRAME_HEX");
        }
#endif
